...
class TurtleState:
    def __init__(self, pos, step, angle, defwidth):
        self.pos = pos
        self.step = step
        self.angle = angle
    ...
    # Clone (?)
    def clone(self):
        clone = TurtleState(self.pos, self.step, self.angle, self.defwidth)
        return clone

    def __repr__(self):
        toPrint = str(self.pos) + ", " + str(self.step) + ", " + str(self.angle) + ", " + str(self.defwidth)
        return toPrint
...
class Turtle:
    ...
    def push(self):
        ...
        t = TurtleState(pos, self.length, self.angle, self.defwidth)
        self.s.push(t.clone())
...
if __name__=='__main__':
    ...
    #
    win = GraphWin('Lindenmayer System', 1000, 400)
    win.yUp()
    ...
    t.defwidth = 12
    t.drawLS(tree, 10, 800, 30, pi/2)

    k = LS(10,pi/2)
    k.setAxiom('F')
    k.addRule('F', 'F+f++f-F--FF-f+')
    k.addRule('f', '-F+ff++f+F--F-f')

    t.defwidth = 1
    t.drawLS(k, 4, 400, 300, 0.942478)
